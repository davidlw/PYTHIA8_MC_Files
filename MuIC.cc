...
//  pythia.readString("HiggsSM:all = on");
  ...
  TH2D* higgsetapthist = new TH2D("higgsetapthist",";#eta;pT [GeV]", 80, -8., 8., 400, 0., 100.);
...
//  TFile *file = TFile::Open("pytree_mup_ycut001.root","recreate");
  TFile *file = TFile::Open("pytree_mup_muic_dp05_dang2_Q21_higgs.root","recreate");
...
*/
    ...
    for (int i = 0; i < event.size(); ++i)
    {
      ...
      if(event[i].id()==25) { cout<<"found a Higgs!"<<endl; higgsetapthist->Fill(event[i].eta(),event[i].pT()); }
continue;
